...
for j in range(NumberOfAllEvents//NEvent):
    ...
    test_TopoClus = np.array([ MakeTopoCluster(test_Layer[i_img], test_Noise_Layer[i_img]) for i_img in range( len(test_Layer) )  ])
    ...
    print('Frac shape : ', frac_test_Nu_Layer.shape)


    if (j==0):
        hfile = h5py.File('/storage/agrp/sanmay/PFLOW_ANALYSIS/RectangularGeo/CROSS_CHECK/TOPO_FILES/'+nameFile+'.hdf5','w')
        ...
        dsettesttopo = testgrp.create_dataset ("topocluster" , data = test_TopoClus,        compression = "lzf", chunks=(10, 6, 256, 256), maxshape=(20000,7,256,256))
        ...
        with h5py.File('/storage/agrp/sanmay/PFLOW_ANALYSIS/RectangularGeo/CROSS_CHECK/TOPO_FILES/'+nameFile+'.hdf5', 'a') as hf:
            '''
            ...
            hf["test"]["topocluster"].resize((hf["test"]["topocluster"].shape[0] + test_TopoClus.shape[0]), axis = 0)
            hf["test"]["topocluster"][-test_TopoClus.shape[0]:] = test_TopoClus
    ...
    del frac_test_Nu_Layer, test_RealRes, test_TopoClus
