...
nameFile = 'Outputfile_SuperRes' + str(sys.argv[1])
...
def MakeSuperResolution(image, noise_image, tr_image, doTopo=False) : 
    
    x1 = SumPixel(image[0], noise_image[0], size= LayerPix[0] )
    #x1 = np.reshape( x1, tuple( [1] + list(x1.shape) ) )
    
    x2 = SumPixel(image[1], noise_image[1], size= LayerPix[0] )
    #x2 = np.reshape( x2, tuple( [1] + list(x2.shape) ) )
    
    x3 = SumPixel(image[2], noise_image[2], size= LayerPix[0] )
    #x3 = np.reshape( x3, tuple( [1] + list(x3.shape) ) )
     
    x4 = SumPixel(image[3], noise_image[3], size= LayerPix[0] )
    #x4 = np.reshape( x4, tuple( [1] + list(x4.shape) ) )
    
    x5 = SumPixel(image[4], noise_image[4], size= LayerPix[0] )
    #x5 = np.reshape( x5, tuple( [1] + list(x5.shape) ) )
    
    x6 = SumPixel(image[5], noise_image[5], size= LayerPix[0] )
    #x6 = np.reshape( x6, tuple( [1] + list(x6.shape) ) )
    ...
    if(doTopo) : 
        Proto = tools_generic.Clustering( x1[ 0:LayerPix[0], 0:LayerPix[0]  ], 
                                          x2[ 0:LayerPix[0], 0:LayerPix[0]  ],
                                          x3[ 0:LayerPix[0], 0:LayerPix[0]  ],
                                          x4[ 0:LayerPix[0], 0:LayerPix[0]  ],
                                          x5[ 0:LayerPix[0], 0:LayerPix[0]  ],
                                          x6[ 0:LayerPix[0], 0:LayerPix[0]  ],
                                        )
...
for j in range(NumberOfAllEvents//NEvent):
    ...
    Ch_Layer_Orig = [f['EventTree'].array('cellCh_Energy', entrystart =j * NEvent+ i*NSlice, entrystop =j * NEvent + (i+1)*NSlice) for i in range(0,int(NEvent/NSlice))]
    Ch_Layer_Orig = np.stack(Ch_Layer_Orig, axis=0)
    Ch_Layer_Orig = Ch_Layer_Orig.reshape(NEvent, 6, true_pix, true_pix)


    Nu_Layer_Orig = [f['EventTree'].array('cellNu_Energy', entrystart =j * NEvent+ i*NSlice, entrystop =j * NEvent + (i+1)*NSlice) for i in range(0,int(NEvent/NSlice))]
    ...
    Noise_Layer = [f['EventTree'].array('Noise_cell_Energy', entrystart =j * NEvent+ i*NSlice, entrystop =j * NEvent + (i+1)*NSlice) for i in range(0,int(NEvent/NSlice))]
    Noise_Layer = np.stack(Noise_Layer, axis=0)
    Noise_Layer = Noise_Layer.reshape(NEvent, 6, true_pix, true_pix)

    # --- adding the track --- #
    Trk_X_pos = [f['EventTree'].array('Trk_X_pos', entrystart = j * NEvent+ i*NSlice, entrystop = j * NEvent + (i+1)*NSlice) for i in range(0, int(NEvent/NSlice)  )]
    Trk_Y_pos = [f['EventTree'].array('Trk_Y_pos', entrystart = j * NEvent+ i*NSlice, entrystop = j * NEvent + (i+1)*NSlice) for i in range(0, int(NEvent/NSlice)  )]
    ...
    Track_Energy = [f['EventTree'].array('Smeared_Ch_Energy', entrystart = j * NEvent+ i*NSlice, entrystop = j * NEvent + (i+1)*NSlice) for i in range(0,  int(NEvent/NSlice)  )]
    ...
    train_size, val_size = int(0.7 * NEvent), int(0.2 * NEvent)
    ...
    train_SuperRes = np.array([ MakeSuperResolution(train_Layer[i_img], train_Noise_Layer[i_img], train_Track_Layer[i_img], doTopo=True) for i_img in range( len(train_Layer) )  ])
    test_SuperRes = np.array([ MakeSuperResolution(test_Layer[i_img], test_Noise_Layer[i_img], test_Track_Layer[i_img], doTopo=True) for i_img in range( len(test_Layer) )  ])
    val_SuperRes = np.array([ MakeSuperResolution(val_Layer[i_img], val_Noise_Layer[i_img], val_Track_Layer[i_img], doTopo=True) for i_img in range( len(val_Layer) )  ])

    train_Nu_SuperRes = np.array([ MakeSuperResolution(train_Nu_Layer[i_img], np.zeros([6, orig_pix, orig_pix]), train_Track_Layer[i_img]) for i_img in range( len(train_Nu_Layer) )  ])
    test_Nu_SuperRes = np.array([ MakeSuperResolution(test_Nu_Layer[i_img], np.zeros([6, orig_pix, orig_pix]), test_Track_Layer[i_img]) for i_img in range( len(test_Nu_Layer) )  ])
    val_Nu_SuperRes = np.array([ MakeSuperResolution(val_Nu_Layer[i_img], np.zeros([6, orig_pix, orig_pix]), val_Track_Layer[i_img]) for i_img in range( len(val_Nu_Layer) )  ])
    ...
    Pair_train_Nu_Layer = np.array( list(zip(train_Nu_SuperRes, train_SuperRes) ) )
    Pair_val_Nu_Layer = np.array( list(zip(val_Nu_SuperRes, val_SuperRes) ) )
    Pair_test_Nu_Layer = np.array( list(zip(test_Nu_SuperRes, test_SuperRes) ) )
    ...
    del Pair_train_Nu_Layer; del Pair_val_Nu_Layer; del Pair_test_Nu_Layer;
