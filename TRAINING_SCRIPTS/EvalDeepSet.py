...
model = model = DeepSet( feature_dims = [6, 12, 8, 4], output_classes = 1)
model = model.to(dev)


model.load_state_dict(torch.load('model_DeepSet_'+nameSourceFile+'.pt'))
model.eval()
...
hf = h5py.File('TOPO_FILES/PredictionFile_DeepSet_' + nameSourceFile + '_V1.h5', 'w')
