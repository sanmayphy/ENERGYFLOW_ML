...
nameFile = 'Outputfile_NoTopo_' + str(sys.argv[1])
...
for j in range(NumberOfAllEvents//NEvent):
    ...
    train_RealRes = np.array([ MakeRealResolution(train_Layer[i_img], train_Noise_Layer[i_img], train_Track_Layer[i_img], doTopo=False) for i_img in range( len(train_Layer) )  ])

    test_RealRes = np.array([ MakeRealResolution(test_Layer[i_img], test_Noise_Layer[i_img], test_Track_Layer[i_img], doTopo=False) for i_img in range( len(test_Layer) )  ])
    val_RealRes = np.array([ MakeRealResolution(val_Layer[i_img], val_Noise_Layer[i_img], val_Track_Layer[i_img], doTopo=False) for i_img in range( len(val_Layer) )  ])
