...
#         print('end_hr : ', end_hr)
        ...
        neu_energies_highres_orig = neu_energies_highres
        ...
        g_hr.ndata['neu_energy_orig'] = neu_energies_highres_orig[:, None]
...
def LossFunction(bg_hr_pr, gr_hr_tar, vec, theta, phi) : 
    ...
    tar_neu_energy_orig = gr_hr_tar.ndata['neu_energy_orig'] 
    # tar_ne_en = tar_ne_en   
    # pred_ne_en = pred_ne_en

    
    wt_avg = torch.sum( tar_neu_energy_orig.to(dev) * torch.abs( pred_ne_en.to(dev) - tar_ne_en.to(dev) ) ** 2  ) 
    
    wt_avg = wt_avg / torch.sum( tar_neu_energy_orig.to(dev)  )
    
    theta_pr = vec[:,0]
    phi_pr   = vec[:,1]
    mass_fr  = vec[:,2]
    
    del_th  = torch.sum( ( (theta_pr - theta)/theta ) ** 2 ) * 5
    del_phi = torch.sum( ( (phi_pr - phi)/phi ) ** 2 )
    del_m   = torch.sum( (mass_fr - 1) ** 2  ) * 2
    
    total_loss = wt_avg + del_th #+ del_m #+ del_phi 
...
train_data = SuperResDataset('training_set.h5', ndata=10000)
valid_data = SuperResDataset('validation_set.h5', ndata=2000)
...
for epoch in range(1, n_epochs+1):
    ...
    if valid_loss <= valid_loss_min:
        ...
        valid_loss_min,
        valid_loss))
        torch.save(model.state_dict(), 'model_GraphUNet_PreProc.pt')
