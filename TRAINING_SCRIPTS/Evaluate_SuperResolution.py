...
def Mass_distance(pos_,ene_, x_orig, y_orig):
  Mass = 0
  Distance = 0
...
fnn = h5py.File('PredictionFile_SR_PI0_' + nameSourceFile + '.h5')
...
for ev in range(Events):
 ...
 x_orig = fnn["Trk_X_indx"][ev][0] * 2.
 y_orig = fnn["Trk_Y_indx"][ev][0] * 2.
 ...
 if np.sum(Th_Ev) < 500 or np.sum(Th_Ev) > 5000: continue 
 for lay in range(3):
  Lay_Max_pred      = [0,0,0] 
  Lay_Max_truth     = [0,0,0]
...
## TODO: add event display for single events
  ...
  #################################################################################
 reco_mass, reco_distance   = Mass_distance(pos_,ene_, x_orig, y_orig)
 truth_mass, truth_distance = Mass_distance(posT_,eneT_, x_orig, y_orig)
 truth_mass_Sta, truth_distance_Sta = Mass_distance(posStan_,eneStan_,  x_orig, y_orig)
...
f_o=ROOT.TFile("Epred_out_SuperResPosition_" + nameSourceFile + ".root","recreate") 
for ly in range(3):
 ED_n_Plot[ly].Write()
 ED_nt_Plot[ly].Write()
