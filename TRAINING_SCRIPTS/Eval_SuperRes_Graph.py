...
model.load_state_dict(torch.load('model_GraphUNet_Short_sc2.pt'))
model.eval()
...
with tqdm.tqdm(test_loader, ascii=True) as tq:
    ...
    for gr ,gr_hr, gr_out_hr, pi0_theta, pi0_phi in tq:
        ...
        for ig in range(batch_len) :
            ...
            total_en     = gr_lr.ndata['energy']
            tar_ne_en_hr = gr_hr_tar.ndata['neu_energy']
            pred_ne_en_hr = gr_hr_pr.ndata['neu_energy']
...
print( 'Avg Er : ', np.mean( (PR_EN - TR_EN)/TR_EN ) )
print( 'Sigma Er : ', np.std( (PR_EN - TR_EN)/TR_EN ) )
